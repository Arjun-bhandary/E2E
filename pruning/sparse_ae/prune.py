...
# Hardcoded Paths for configs
CHECKPOINT_PATH    = "/raid/home/dgx1736/Arush1/sparse_autoencoder/finetuned_classifier.pt"
DATA_PATH          = "/raid/home/dgx1736/Arush1/Dataset_Specific_labelled.h5"
OUTPUT_DIR         = "/raid/home/dgx1736/Arush1/pruning/sparse_ae"
...
def estimate_flops(sparsity_ratio=0.0, avg_active_pixels=1500):
    """
    Identical architecture to Sparse ResNet MAE → same FLOPS formula.
    The only difference is the pretrained weights (L1+KL vs MAE).
    """
    density = 1.0 - sparsity_ratio
    base_ch = 64; K = 3
    n0 = avg_active_pixels
    n1 = n0 * 0.6; n2 = n1 * 0.5; n3 = n2 * 0.4

    flops = 0
    flops += 2 * K*K * 8 * base_ch * n0                                   # stem
    for _ in range(2): flops += 2 * (2 * K*K * base_ch * base_ch * n0)    # stage1
    flops += 2 * K*K * base_ch * base_ch * n0                               # down1

    flops += 2 * K*K * base_ch * base_ch*2 * n1                            # stage2 block1 conv1
    flops += 2 * K*K * base_ch*2 * base_ch*2 * n1                          # stage2 block1 conv2
    flops += 2 * 1 * base_ch * base_ch*2 * n1                              # stage2 block1 skip
    flops += 2 * (2 * K*K * base_ch*2 * base_ch*2 * n1)                    # stage2 block2
    flops += 2 * K*K * base_ch*2 * base_ch*2 * n1                          # down2
    ...
    flops += 2 * K*K * base_ch*4 * base_ch*4 * n2                          # down3
    ...
    flops += 2*512*256 + 2*256*64 + 2*64*1                                 # classifier
    flops *= density
    return flops
...
def plot_results(results, output_dir):
    ...
    ax.plot(gflops, errors, color='#16a34a', marker='^', markersize=9,
            linewidth=2.5, label='Sparse Autoencoder (L1+KL)', zorder=5)
    ...
    for i, (gf, er, pr) in enumerate(zip(gflops, errors, ratios)):
        ...
        ax.annotate(f'{pr:.0%}', (gf, er), textcoords='offset points',
                    xytext=offset, fontsize=8, color='#16a34a', alpha=0.85)

    ax.set_xlabel('GFLOPS (estimated)', fontsize=13, fontweight='bold')
    ax.set_ylabel('Error (1 − Accuracy)', fontsize=13, fontweight='bold')
    ax.set_title('Sparse Autoencoder (L1+KL) — Pruning: Error vs FLOPS',
                 fontsize=14, fontweight='bold')
    ax.legend(fontsize=11, loc='upper left')
    ax.grid(True, alpha=0.3)
    ax.set_ylim(bottom=0)
    plt.tight_layout()
    path = os.path.join(output_dir, 'sparse_ae_error_vs_flops.png')
...
def main():
    ...
    # ── Load model ────────────────────────────────────────────────────────
    model = SparseAEClassifier(IN_CHANNELS, ENCODER_DIM, 0.5).to(DEVICE)
    ...
    print(f"Loaded Sparse Autoencoder (L1+KL) | Params: {n_params:,}")
    ...
    # ── Save ──────────────────────────────────────────────────────────────
    json_path = os.path.join(OUTPUT_DIR, 'sparse_ae_pruning_results.json')
