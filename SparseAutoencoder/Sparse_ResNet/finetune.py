...
print(f"Architecture: Sparse Autoencoder (L1+KL sparsity pretrained)")
...
def save_roc_plot(fpr, tpr, auc_val, path):
    ...
    ax.set_title('ROC — Sparse Autoencoder'); ax.legend(); ax.grid(True, alpha=0.3)
...
def save_loss_plot(history, path):
    ...
    ax.set_xlabel('Epoch'); ax.set_ylabel('BCE Loss'); ax.set_title('Loss (Sparse AE)')
...
def save_auc_plot(history, path):
    ...
    ax.axvline(FREEZE_EPOCHS+0.5, color='green', ls='--', alpha=0.5, label='Unfreeze')
    ax.set_xlabel('Epoch'); ax.set_ylabel('AUC'); ax.set_title('ROC-AUC (Sparse AE)')
    ax.legend(); ax.grid(True, alpha=0.3)
    plt.tight_layout(); plt.savefig(path, dpi=150); plt.close()
...
def save_acc_plot(history, path):
    ...
    ax.axvline(FREEZE_EPOCHS+0.5, color='green', ls='--', alpha=0.5, label='Unfreeze')
    ax.set_xlabel('Epoch'); ax.set_ylabel('Accuracy'); ax.set_title('Accuracy (Sparse AE)')
    ax.legend(); ax.grid(True, alpha=0.3)
    plt.tight_layout(); plt.savefig(path, dpi=150); plt.close()
...
def save_cm_plot(cm, path):
    ...
    ax.set_xlabel('Predicted'); ax.set_ylabel('True'); ax.set_title('Confusion Matrix (Sparse AE)')
...
if __name__ == '__main__':
    ...
    model = SparseAEClassifier(IN_CHANNELS, ENCODER_DIM, DROPOUT).to(DEVICE)
    if os.path.exists(ENCODER_PATH):
        ...
        print(f"Loaded encoder (sparse AE pretrained) | Missing: {len(m)} | Unexpected: {len(u)}")
...
#   Final evaluation
    ...
    print(f"\n  Baseline MAE:           AUC = 0.9602")
    print(f"  Sparse Autoencoder:     AUC = {fm['auc']:.4f}")
    print(f"  Δ (SAE - MAE):          {fm['auc'] - 0.9602:+.4f}")
    ...
    summary = dict(best_epoch=ckpt['epoch'], auc=fm['auc'], accuracy=fm['accuracy'],
                   ...
                   architecture='sparse_autoencoder_l1_kl',
