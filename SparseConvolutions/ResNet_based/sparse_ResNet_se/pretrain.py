...
class UnlabelledSparseJetDataset(Dataset):
    def __init__(self, path, threshold=0.0):
        ...
        self.n_samples = Configs.UNLABELED_N_SAMPLES
    ...
    def __getitem__(self, idx):
        with open(self.path, 'rb') as f:
            f.seek(Configs.UNLABELED_JET_OFFSET + idx * self.sample_bytes)
...
def mae_collate_fn(batch, mask_ratio=Configs.MASK_RATIO, threshold=Configs.THRESHOLD):
...
class SparseSEBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        se = self.sigmoid(self.fc2(self.relu(self.fc1(pooled))))  
        scale = se[bidx]  
...
def _vectorized_lookup(dec_indices, dec_feats, query_coords, device):
    ...
    max_hash = Configs.BATCH_SIZE * 15625 + 1
...
class SparseMAE_SE(nn.Module):
    ...
    def forward(self, vis_coords, vis_feats, mask_coords, mask_feats, batch_size):
        ...
        x = spconv.SparseConvTensor(all_feats, all_coords, Configs.SPATIAL_SIZE, batch_size)
        z = self.encoder(x)
        decoded = self.decoder(z)

        rf, _, rqi = _vectorized_lookup(decoded.indices, decoded.features, mask_coords, vis_feats.device)
...
def train_one_epoch(model, loader, optimizer):
    ...
    for batch in tqdm(loader, desc='Train', leave=False):
        vc, vf = batch['vis_coords'].to(Configs.DEVICE), batch['vis_feats'].to(Configs.DEVICE)
        mc, mf = batch['mask_coords'].to(Configs.DEVICE), batch['mask_feats'].to(Configs.DEVICE)
...
if __name__ == '__main__':
    ...
    t0 = time.time()
    dataset = UnlabelledSparseJetDataset(Configs.UNLABELED_PATH, Configs.THRESHOLD)
    n = len(dataset)
    ...
    model = SparseMAE_SE(Configs.IN_CHANNELS, Configs.ENCODER_DIM, Configs.DECODER_DIM).to(Configs.DEVICE)
    
    optimizer = torch.optim.AdamW(model.parameters(), lr=Configs.LR, weight_decay=Configs.WEIGHT_DECAY)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=Configs.EPOCHS, eta_min=1e-6)
    ...
    for epoch in range(1, Configs.EPOCHS + 1):
        ...
        if vl < best_val:
            best_val = vl
            pat = 0
            torch.save(model.encoder.state_dict(), os.path.join(Configs.SAVE_DIR, 'sparse_mae_encoder.pt'))
            torch.save(model.state_dict(), os.path.join(Configs.SAVE_DIR, 'sparse_mae_full.pt'))
            mk = ' ← best'
        else: 
            pat += 1
            
        print(f"{epoch:<5} {tl:<12.6f} {vl:<12.6f} {lr:<10.2e}{mk}")
        if pat >= Configs.PATIENCE:
            break

    with open(os.path.join(Configs.SAVE_DIR, 'pretrain_history.json'), 'w') as f:
        json.dump(history, f, indent=2)
    save_recon_loss_plot(history, os.path.join(Configs.SAVE_DIR, 'pretrain_recon_loss.jpg'))
