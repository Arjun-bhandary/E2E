#!/usr/bin/env python3
"""
finetune.py — Sparse ResNet MAE Fine-tuning (cleaned)
=====================================================
Fine-tune pretrained Sparse ResNet encoder for binary jet classification.

Data loading: Lazy per-sample raw file read (avoids numpy large-array bug)
Plots: Individual .jpg files for GitHub
...
ENCODER_PATH   = '/raid/home/dgx1736/Arush1/sparse_resnet/sparse_mae_encoder.pt'
SAVE_DIR       = '/raid/home/dgx1736/Arush1/sparse_resnet'
...
# ═══════════════════════════════════════════════════════════════════════════════
# 1. DATASET — lazy per-sample raw file read
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# 2. MODEL — Sparse ResNet Encoder (nn.ModuleList)
# ═══════════════════════════════════════════════════════════════════════════════
...
def save_roc_plot(fpr, tpr, auc_val, path):
    ...
    ax.set_title('ROC Curve — Sparse ResNet + MAE'); ax.legend(); ax.grid(True, alpha=0.3)
...
def save_auc_plot(history, path):
    ...
    ax.axvline(FREEZE_EPOCHS+0.5, color='green', ls='--', alpha=0.5, label='Unfreeze')
    ax.set_xlabel('Epoch'); ax.set_ylabel('AUC'); ax.set_title('ROC-AUC')
    ax.legend(); ax.grid(True, alpha=0.3)
    plt.tight_layout(); plt.savefig(path, dpi=150); plt.close()
    print(f"Saved: {path}")
...
if __name__ == '__main__':
    ...
    if os.path.exists(ENCODER_PATH):
        ...
        print(f"Loaded encoder | Missing: {len(m)} | Unexpected: {len(u)}")
    ...
    print(f"  Baselines → Sparse baseline: 0.9565 | Dense AE: 0.9610")
    print(f"  Δ vs baseline: {fm['auc']-0.9565:+.4f} | Δ vs dense AE: {fm['auc']-0.9610:+.4f}")
    ...
    summary = dict(best_epoch=ckpt['epoch'], auc=fm['auc'], accuracy=fm['accuracy'],
                   ...
                   inv_fpr_07=fm['inv_fpr_at_tpr07'], confusion_matrix=fm['confusion_matrix'],
